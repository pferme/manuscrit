...
\abstract{We address the problem of coding for classical broadcast channels, which entails maximizing the success probability that can be achieved by sending a fixed number of messages over a broadcast channel. For point-to-point channels, Barman and Fawzi found in~\cite{BF18} an $(1-e^{-1})$-approximation algorithm running in polynomial time, and showed that it is \textrm{NP}-hard to achieve a strictly better approximation ratio. Furthermore, these algorithmic results were at the core of their proof that non-signaling assistance does not change the capacity of point-to-point channels. It is natural to if similar results hold for broadcast channels, exploiting links between approximation algorithms of the channel coding problem and the non-signaling assisted capacity region.
...
\begin{proof}
  It follows directly from the definitions $W_1(y_1|x) := \sum_{y_2} W(y_1y_2|x)$ and $W_2(y_2|x) := \sum_{y_1} W(y_1y_2|x)$.
\end{proof}
...
Thus, when receivers are given non-signaling assistance, it means that the product $d_1(j_1|y_1)d_2(j_2|y_2)$ is replaced by the non-signaling box $d(j_1j_2|y_1y_2)$. We define the joint and sum success probabilities $\mathrm{S}^{\mathrm{NS}_{\text{decs}}}(W,k_1,k_2)$ and $\mathrm{S}_{\text{sum}}^{\mathrm{NS}_{\text{decs}}}(W,k_1,k_2)$ according to this:

\begin{equation}
  \begin{aligned}
    \mathrm{S}^{\mathrm{NS}_{\text{decs}}}(W,k_1,k_2) := &&\underset{e,d_1,d_2}{\maxi} &&& \frac{1}{k_1k_2}\sum_{i_1,i_2,x,y_1,y_2} W(y_1y_2|x)e(x|i_1i_2)d(i_1i_2|y_1y_2)\\
    \Big(\text{resp. }\mathrm{S}_{\text{sum}}^{\mathrm{NS}_{\text{decs}}}(W,k_1,k_2) := &&\underset{e,d_1,d_2}{\maxi} &&& \frac{1}{k_1k_2}\sum_{i_1,i_2,x,y_1,y_2} W(y_1y_2|x)e(x|i_1i_2)\frac{\sum_{j_2}d(i_1j_2|y_1y_2) + \sum_{j_1}d(j_1i_2|y_1y_2)}{2}\Big)\\
...
The corresponding capacity regions $\mathcal{C}^{\mathrm{NS}_{\text{decs}}}(W)$ and $\mathcal{C}^{\mathrm{NS}_{\text{decs}}}_{\text{sum}}(W)$ are defined as before:

\begin{defi}[Capacity Region $\mathcal{C}^{\mathrm{NS}_{\text{decs}}}(W)$ (resp. $\mathcal{C}^{\mathrm{NS}_{\text{decs}}}_{\text{sum}}(W)$) of a broadcast channel $W$]
  A rate pair $(R_1,R_2)$ is achievable (resp. sum-achievable) with non-signaling assistance between the decoders if:
  \[ \underset{n \rightarrow +\infty}{\lim} \mathrm{S}^{\mathrm{NS}_{\text{decs}}}(W^{\otimes n},\ceil{2^{R_1n}},\ceil{2^{R_2n}}) = 1 \ . \]
  \[ \text{(resp. }\underset{n \rightarrow +\infty}{\lim} \mathrm{S}^{\mathrm{NS}_{\text{decs}}}_{\text{sum}}(W^{\otimes n},\ceil{2^{R_1n}},\ceil{2^{R_2n}}) = 1 )\ . \]
  We define the joint (resp. sum) capacity region with non-signaling assistance between the decoders $\mathcal{C}^{\mathrm{NS}_{\text{decs}}}(W)$ (resp. $\mathcal{C}^{\mathrm{NS}_{\text{decs}}}_{\text{sum}}(W)$) as the closure of the set of all rate pairs achievable (resp. sum-achievable) with non-signaling assistance between the decoders.
\end{defi}
...
\begin{prop}
  ...
  For any broadcast channel $W$, $\mathcal{C}_{\text{sum}}^{\mathrm{NS}_{\text{decs}}}(W)= \mathcal{C}^{\mathrm{NS}_{\text{decs}}}(W)$
\end{prop}
...
\[ \mathrm{S}^{\mathrm{NS}_{\text{decs}}}(W,k_1,k_2,e,d) := \frac{1}{k_1k_2}\sum_{i_1,i_2,x,y_1,y_2} W(y_1y_2|x)e(x|i_1i_2)d(i_1i_2|y_1y_2) \ . \]

This should be compared to the sum success probability of encoding and decoding correctly with those:
\[ \mathrm{S}_{\text{sum}}^{\mathrm{NS}_{\text{decs}}}(W,k_1,k_2,e,d) := \frac{1}{k_1k_2}\sum_{i_1,i_2,x,y_1,y_2} W(y_1y_2|x)e(x|i_1i_2)\left[\frac{\sum_{j_2}d(i_1j_2|y_1y_2)+\sum_{j_1}d(j_1i_2|y_1y_2)}{2}\right] \ . \]
...
\[ \mathrm{E}^{\mathrm{NS}_{\text{decs}}}(W,k_1,k_2,e,d) = \frac{1}{k_1k_2}\sum_{i_1,i_2,x,y_1,y_2} W(y_1y_2|x)e(x|i_1i_2)\left[1-d(i_1i_2|y_1y_2)\right] \ , \]
and:
\[ \mathrm{E}_{\text{sum}}^{\mathrm{NS}_{\text{decs}}}(W,k_1,k_2,e,d) = \frac{1}{k_1k_2}\sum_{i_1,i_2,x,y_1,y_2} W(y_1y_2|x)e(x|i_1i_2)\left[\frac{1-\sum_{j_2}d(i_1j_2|y_1y_2)}{2}+\frac{1-\sum_{j_1}d(j_1i_2|y_1y_2)}{2}\right] \ . \]
...
\[\mathrm{E}_{\text{sum}}^{\mathrm{NS}_{\text{decs}}}(W,k_1,k_2,e,d) \leq \mathrm{E}^{\mathrm{NS}_{\text{decs}}}(W,k_1,k_2,e,d) \leq 2\mathrm{E}_{\text{sum}}^{\mathrm{NS}_{\text{decs}}}(W,k_1,k_2,e,d) \ ,\]
and by maximizing over all $e$ and $d$:
\[\mathrm{E}_{\text{sum}}^{\mathrm{NS}_{\text{decs}}}(W,k_1,k_2) \leq \mathrm{E}^{\mathrm{NS}_{\text{decs}}}(W,k_1,k_2) \leq 2\mathrm{E}_{\text{sum}}^{\mathrm{NS}_{\text{decs}}}(W,k_1,k_2) \ .\]
As before, this implies that the capacity regions are the same.
\end{proof}

\begin{theo}
    \label{theo:NSdecoders}
   For any broadcast channel $W$, $\mathcal{C}(W) = \mathcal{C}^{\mathrm{NS}_{\text{decs}}}(W)$
\end{theo}
...
\begin{equation}
  \begin{aligned}
    \mathrm{S}_{\text{sum}}^{\mathrm{NS}_{\text{decs}}}(W,k_1,k_2,e,d) &:= \frac{1}{2k_1k_2}\sum_{i_1,x,y_1} W_1(y_1|x)\left(\sum_{j_2}d(i_1j_2|y_1y_2)\right)\sum_{i_2} e(x|i_1i_2)\\
    &+ \frac{1}{2k_1k_2} \sum_{i_2,x,y_2} W_2(y_2|x)\left(\sum_{j_1}d(j_1i_2|y_1y_2)\right)\sum_{i_1} e(x|i_1i_2) \ .
  \end{aligned}
\end{equation}

Thus, by choosing $d_1(j_1|y_1) := \sum_{j_2}d(j_1j_2|y_1y_2)$ and $d_2(j_2|y_2) := \sum_{j_1}d(j_1j_2|y_1y_2)$, which are well-defined since $d$ is a non-signaling box, one gets that $\mathrm{S}_{\text{sum}}(W,k_1,k_2,e,d_1,d_2) = \mathrm{S}_{\text{sum}}^{\mathrm{NS}_{\text{decs}}}(W,k_1,k_2,e,d)$. By optimizing over all $e$ and $d$, one gets $\mathrm{S}_{\text{sum}}^{\mathrm{NS}_{\text{decs}}}(W,k_1,k_2) \leq \mathrm{S}_{\text{sum}}(W,k_1,k_2)$. Since the inequality is obvious in the other direction, as $d(j_1j_2|y_1y_2) := d_1(j_1|y_1)d_2(j_2|y_2)$ is always a non-signaling box, we have that $\mathrm{S}_{\text{sum}}(W,k_1,k_2)=\mathrm{S}_{\text{sum}}^{\mathrm{NS}_{\text{decs}}}(W,k_1,k_2)$. This implies in particular that the capacity regions are the same, ie. $\mathcal{C}_{\text{sum}}(W)=\mathcal{C}_{\text{sum}}^{\mathrm{NS}_{\text{decs}}}(W)$

Finally, since $\mathcal{C}(W) = \mathcal{C}_{\text{sum}}(W)$ by Proposition~\ref{prop:capacitysum} and $\mathcal{C}_{\text{sum}}^{\mathrm{NS}_{\text{decs}}}(W)= \mathcal{C}^{\mathrm{NS}_{\text{decs}}}(W)$ by Proposition~\ref{prop:NSdecoderssum}, we get that $\mathcal{C}(W) = \mathcal{C}^{\mathrm{NS}_{\text{decs}}}(W)$.
\end{proof}
...
Since it is given as a linear program, the complexity of computing $\mathrm{S}_{\text{sum}}^{\mathrm{NS}}(W,k_1,k_2)$ is polynomial in the number of variables and constraints (see for instance Section 7.1 of~\cite{LinearProgramming}), which is a polynomial in $|\mathcal{X}|,|\mathcal{Y}_1|,|\mathcal{Y}_2|, k_1$ and $k_2$.
...
In this section, we will address the question of the approximability of $\mathrm{S}(W,k_1,k_2)$, in the restricted scenario of a deterministic broadcast channel $W$.

We say that $W$ is deterministic if $\forall x,y_1,y_2, W(y_1y_2|x) \in \{0,1\}$. We can then define $(W_1(x),W_2(x))$ as the only pair $(y_1,y_2)$ such that $W(y_1y_2|x)=1$, which exists uniquely as $W$ is a conditional probability distribution. We have then:

\begin{equation}
  \begin{aligned}
    \mathrm{S}(W,k_1,k_2) = &&\underset{e,d_1,d_2}{\maxi} &&& \frac{1}{k_1k_2}\sum_{x,i_1,i_2}e(x|i_1i_2)d_1(i_1|W_1(x))d_2(i_2|W_2(x))\\
    &&\st &&& \sum_{x \in \mathcal{X}} e(x|i_1i_2) = 1, \forall i_1 \in [k_1], i_2 \in [k_2]\\
...
\subsection{Reformulation as a Graph Problem}
A deterministic channel $W$, up to a permutation of elements of $\mathcal{X}$, is characterized by the following bipartite graph:
...
Indeed, permuting the elements of $\mathcal{X}$ won't change $G_W$, and permuting elements of $\mathcal{Y}_1$ or $\mathcal{Y}_2$ will result in an isomorphic graph. These permutations do not change $\mathrm{S}(W,k_1,k_2)$, so we can ignore them in order to compute this quantity.

As a consequence, the quantity $k_1k_2\mathrm{S}(W,k_1,k_2)$ can be seen as the solution of the following bipartite graph problem on $G_W, k_1$ and $k_2$, which we call \textsc{DensestQuotientGraph}: given a bipartite graph $G$ and integers $k_1,k_2$, maximize, over all quotient graphs of $G$ in $k_1$ parts on the left and $k_2$ parts on the right, the resulting number of edges.

Recall that the bipartite graph $G$ quotiented by partitions $\mathcal{P}_1,\mathcal{P}_2$ is given by:
\[ G^{\mathcal{P}_1,\mathcal{P}_2} := \left(\mathcal{P}_1 \sqcup \mathcal{P}_2, \{(p_1,p_2) \in \mathcal{P}_1 \times \mathcal{P}_2: E^G \cap (p_1 \times p_2) \not= \emptyset\}\right) \ .\]

So formally, \textsc{DensestQuotientGraph} is the following problem:
\[  \textsc{DensestQuotientGraph}(G,k_1,k_2) := \underset{\mathcal{P}_1 \text{ in $k_1$ parts},\mathcal{P}_2 \text{ in $k_2$ parts}}{\maxi} \ e_G(\mathcal{P}_1,\mathcal{P}_2) := |E^{G^{\mathcal{P}_1,\mathcal{P}_2}}| \ . \]

Let us show the equivalence with the broadcast channel coding problem:

\begin{prop}
  \label{prop:BCCisDQG}
  We have $k_1k_2\mathrm{S}(W,k_1,k_2) = \textsc{DensestQuotientGraph}(G_W,k_1,k_2)$.
\end{prop}
\begin{proof}
An optimal solution achieving $\mathrm{S}(W,k_1,k_2)$ can be obtained with extremal points as the optimization program defining $\mathrm{S}(W,k_1,k_2)$ is convex. We can thus assume that $e,d_1,d_2$ are functions. Therefore, $d_1$ defines a partition of $\mathcal{Y}_1$, $d_2$ defines a partition of $\mathcal{Y}_2$, and then $e$ denotes the choice of which edge between partition $i_1$ and $i_2$ one choose. Indeed, with $\mathcal{P}_1,\mathcal{P}_2$ corresponding partitions of $\mathcal{Y}_1,\mathcal{Y}_2$ in $k_1,k_2$ parts:
\begin{equation}
  \begin{aligned}
    k_1k_2\mathrm{S}(W,k_1,k_2) = &&\underset{e,d_1,d_2}{\maxi} &&&\sum_{x,i_1,i_2} \mathbbm{1}_{x=e(i_1,i_2)}\mathbbm{1}_{i_1=d_1(W_1(x))}\mathbbm{1}_{i_2=d_2(W_2(x))}\\
    = &&\underset{e,d_1,d_2}{\maxi} &&&\sum_{i_1,i_2} \mathbbm{1}_{i_1=d_1(W_1(e(i_1,i_2)))}\mathbbm{1}_{i_2=d_2(W_2(e(i_1,i_2)))}\\
    = &&\underset{e,\mathcal{P}_1,\mathcal{P}_2}{\maxi} &&&\sum_{i_1,i_2} \mathbbm{1}_{W_1(e(i_1,i_2)) \in \mathcal{P}_1^{i_1}}\mathbbm{1}_{W_2(e(i_1,i_2)) \in \mathcal{P}_2^{i_2}}\\
    = &&\underset{\mathcal{P}_1,\mathcal{P}_2}{\maxi} &&&\sum_{i_1,i_2} \max_{x \in \mathcal{X}}\mathbbm{1}_{W_1(x) \in \mathcal{P}_1^{i_1}}\mathbbm{1}_{W_2(x) \in \mathcal{P}_2^{i_2}}\\
    = &&\underset{\mathcal{P}_1,\mathcal{P}_2}{\maxi} &&&\sum_{i_1,i_2} \mathbbm{1}_{\exists (y_1,y_2) \in E^{G_W}: y_1 \in \mathcal{P}_1^{i_1}\text{ and } y_2 \in \mathcal{P}_2^{i_2}}\\
    = &&\underset{\mathcal{P}_1,\mathcal{P}_2}{\maxi} &&&\sum_{i_1,i_2} \mathbbm{1}_{E^{G_W} \cap \left(\mathcal{P}_1^{i_1} \times \mathcal{P}_2^{i_2}\right) \not= \emptyset}\\
    = &&\underset{\mathcal{P}_1,\mathcal{P}_2}{\maxi} &&& e_{G_W}(\mathcal{P}_1,\mathcal{P}_2) \ ,\\
    \end{aligned}
\end{equation}
by definition of $e_{G_W}(\mathcal{P}_1,\mathcal{P}_2) = |E^{G_W^{\mathcal{P}_1,\mathcal{P}_2}}| = |\{(p_1,p_2) \in \mathcal{P}_1 \times \mathcal{P}_2: E^{G_W} \cap (p_1 \times p_2) \not= \emptyset\}|$. This concludes the equivalence.
\end{proof}

\subsection{Approximation Algorithm for \textsc{DensestQuotientGraph}}
In this section, we will sort out how hard is \textsc{DensestQuotientGraph}, and thanks to Proposition~\ref{prop:BCCisDQG}, how hard is it to solve the broadcast channel coding problem for deterministic channels.
...
\begin{cor}
  ...
  There exists a polynomial-time $(1-e^{-1})^2$-approximation algorithm for the deterministic broadcast channel coding problem.
\end{cor}
...
We define $e^1_G(\mathcal{P}_1) := e_G(\mathcal{P}_1, \{\{v_2\}: v_2 \in V_2\})$ as the number of edges in $G$ quotiented on the left by the partition $\mathcal{P}_1$ but without quotienting the right part. Let us show that if we take a partition $\mathcal{P}_1$ of $V_1$ uniformly at random, we get:
\[ \mathbb{E}_{\mathcal{P}_1}[e^1_G(\mathcal{P}_1)] \geq \left(1-\left(1-\frac{1}{k_1}\right)^{k_1}\right)\sum_{v_2 \in V_2}\min\left(k_1,\deg(v_2)\right) \geq (1-e^{-1})\max_{\mathcal{P}_1}e^1_G(\mathcal{P}_1) \ .\]

Let us call $G_{v_2} := \left(V^G_1 \sqcup \{v_2\}, E^G \cap \left(V^G_1 \times \{v_2\}\right)\right)$. We have then that $e^1_G(\mathcal{P}_1)=\sum_{v_2 \in V_2}e^1_{G_{v_2}}(\mathcal{P}_1)$, so by linearity of expectation, we have that $\mathbb{E}_{\mathcal{P}_1}[e^1_G(\mathcal{P}_1)] = \sum_{v_2 \in V_2}\mathbb{E}_{\mathcal{P}_1}[e^1_{G_{v_2}}(\mathcal{P}_1)]$. We will focus on the contribution of one particular $v_2$. It is enough to consider only its neighbors $N(v_2)$ as other elements of $V_1$ do not contribute to $e^1_{G_{v_2}}(\mathcal{P}_1)$.

Then, we have that $e^1_{G_{v_2}}(\mathcal{P}_1)=|\{i \in [k_1]: N(v_2) \cap \mathcal{P}_1^i \not= \emptyset  \}|$. Let us call $N(v_2) = \{v_1^1, \ldots, v_1^{\deg(v_2)}\}$. We have that $\mathbb{P}\left(v_1^j \in \mathcal{P}_1^i\right) = \frac{1}{k_1}$ since the partition is taken uniformly at random. Then, we have:

\begin{equation}
  \begin{aligned}
    \mathbb{E}_{\mathcal{P}_1}[e^1_{G_{v_2}}(\mathcal{P}_1)] &= \mathbb{E}_{\mathcal{P}_1}\left[|\{i \in [k_1]: N(v_2) \cap \mathcal{P}_1^i \not= \emptyset  \}|\right]
...
\[ \mathbb{E}_{\mathcal{P}_1}[e^1_G(\mathcal{P}_1)] = \sum_{v_2 \in V_2}\mathbb{E}_{\mathcal{P}_1}[e^1_{G_{v_2}}(\mathcal{P}_1)] = k_1\sum_{v_2 \in V_2}\left(1-\left(1-\frac{1}{k_1}\right)^{\deg(v_2)}\right) \ . \]

However, the function $x \mapsto 1-\left(1-\frac{1}{k_1}\right)^x$ is nondecreasing concave, so we have that:

\begin{equation}
  \begin{aligned}
    \mathbb{E}_{\mathcal{P}_1}[e^1_G(\mathcal{P}_1)] &\geq k_1\sum_{v_2 \in V_2}\left(1-\left(1-\frac{1}{k_1}\right)^{\min(k_1,\deg(v_2))}\right) \geq k_1\frac{\sum_{v_2 \in V_2}\min(k_1,\deg(v_2))}{k_1}\left(1-\left(1-\frac{1}{k_1}\right)^{k_1}\right)\\
    &\geq \left(1-\left(1-\frac{1}{k_1}\right)^{k_1}\right)\sum_{v_2 \in V_2}\min\left(k_1,\deg(v_2)\right) \geq (1-e^{-1})\max_{\mathcal{P}_1}e^1_G(\mathcal{P}_1) \ .
  \end{aligned}
\end{equation}

Indeed, $\sum_x g(f(x)) \geq \frac{\sum_{x}{f(x)}}{M}g(M)$ if $\forall x, f(x) \leq M$ and $g$ nondecreasing concave.
...
\[\max_{\mathcal{P}_2}\sum_{i_2 \in [k_2]}\min\left(k_1,\deg_{G^{\mathcal{P}_2}}(\mathcal{P}_2^{i_2})\right) \ , \]
where $\mathcal{P}_2$ is a partition of $V_2$ in $k_2$ parts, $G^{\mathcal{P}_2} := G^{\{ \{v_1\}: v_1 \in V_1^G\},\mathcal{P}_2}$ the graph quotiented on right by $\mathcal{P}_2$ but without quotienting the left part, and $\deg_{G^{\mathcal{P}_2}}(\mathcal{P}_2^{i_2})$ the degree of the part in $G^{\mathcal{P}_2}$.

Thanks to what was done before, we know that:
\[ \max_{\mathcal{P}_2}\sum_{p_2 \in \mathcal{P}_2}\min\left(k_1,\deg_{G^{\mathcal{P}_2}}(p_2)\right) \geq \max_{\mathcal{P}_2} \max_{\mathcal{P}_1} e^1_{G^{\mathcal{P}_2}}(\mathcal{P}_1) = \max_{\mathcal{P}_1,\mathcal{P}_2} e_G(\mathcal{P}_1,\mathcal{P}_2) \ . \]

Furthermore, given a partition $\mathcal{P}_2$ of $V_2$, we can apply the previous algorithm on $\mathcal{P}_2$ to get $\mathcal{P}_1$ such that $e^1_{G^{\mathcal{P}_2}}(\mathcal{P}_1) \geq (1-e^{-1})\sum_{i_2 \in [k_2]}\min\left(k_1,\deg_{G^{\mathcal{P}_2}}(\mathcal{P}_2^{i_2})\right)$.

We will show that we have a $(1-e^{-1})$ polynomial-time approximation of:

\[ \max_{\mathcal{P}_2}\sum_{p_2 \in \mathcal{P}_2}\sum_{i_2 \in [k_2]}\min\left(k_1,\deg_{G^{\mathcal{P}_2}}(\mathcal{P}_2^{i_2})\right) \ .\]

Thus in all we will get in polynomial time a pair of partition $(\mathcal{P}_1,\mathcal{P}_2)$ such that:

\begin{equation}
  \begin{aligned}
    e_G(\mathcal{P}_1,\mathcal{P}_2) &\geq (1-e^{-1})\sum_{i_2 \in [k_2]}\min\left(k_1,\deg_{G^{\mathcal{P}_2}}(\mathcal{P}_2^{i_2})\right)\\
    &\geq (1-e^{-1})^2\max_{\mathcal{P}_2}\sum_{i_2 \in [k_2]}\min\left(k_1,\deg_{G^{\mathcal{P}_2}}(\mathcal{P}_2^{i_2})\right)\\
    &\geq (1-e^{-1})^2\max_{\mathcal{P}_1,\mathcal{P}_2} e_G(\mathcal{P}_1,\mathcal{P}_2) \ .
  \end{aligned}
\end{equation}

The problem $\max_{\mathcal{P}_2}\sum_{i_2 \in [k_2]}\min\left(k_1,\deg_{G^{\mathcal{P}_2}}(\mathcal{P}_2^{i_2})\right)$ is a particular instance of the submodular welfare problem discussed in~\cite{Vondrak08}. Indeed, the function $h(S) := \min\left(k_1,\deg_{G^{\mathcal{P}_2}}(S)\right)$, for $S \subseteq V_2$, is a nondecreasing submodular function. Thus, we want to maximize $\sum_{i=1}^{k_2}h(S_i)$ where $(S_i)_{i \in [k_2]}$ is a partition of the items in $V_2$ among the $k_2$ bidders. It is a particular case of the submodular welfare problem where each nondecreasing submodular utility weight is the same for all bidders and equal to $h$. Thus, thanks to~\cite{Vondrak08}, there exists a $(1-e^{-1})$ polynomial-time approximation of $\max_{\mathcal{P}_2}\sum_{i_2 \in [k_2]}\min\left(k_1,\deg_{G^{\mathcal{P}_2}}(\mathcal{P}_2^{i_2})\right)$. This concludes the proof.
\end{proof}
...
\[ \left(1 - \frac{k_1^{k_1}e^{-k_1}}{k_1!}\right)\left(1-e^{-nR_1}\right)\left(1-e^{-nR_2}\right)\mathrm{S}^{\mathrm{NS}}(W^{\otimes n},2^{nR_1},2^{nR_1}) \leq \mathrm{S}\left(W^{\otimes n},\frac{2^{nR_1}}{n},\frac{2^{nR_2}}{n}\right)\ . \]

  As $\left(1 - \frac{k_1^{k_1}e^{-k_1}}{k_1!}\right)\left(1-e^{-nR_1}\right)\left(1-e^{-nR_2}\right)$ tends to $1$ when $n$ tends to infinity, we get that $\forall \varepsilon, \exists N \in \mathbb{N}, \forall n\geq N$:
...
\begin{proof}[Proof of Theorem~\ref{theo:NSdet}]
  The proof will be done in three parts:
  \begin{enumerate}
  \item First, we prove that for any partition $\mathcal{P}_2$ of $\mathcal{Y}_2$ in $\ell_2$ parts:
    \[ \mathrm{S}(W,\ell_1,\ell_2) \geq \left(1-\left(1-\frac{1}{\ell_1}\right)^{k_1}\right)\frac{\sum_{i_2 \in [k_2]}\min\left(k_1,\deg_{G_W^{\mathcal{P}_2}}(\mathcal{P}_2^{i_2})\right)}{k_1\ell_2} \ , \]
    with $G_W^{\mathcal{P}_2}$ defined as in the proof of Theorem~\ref{theo:DQGapprox}.
  \item Then, we show that there exists a partition $\mathcal{P}_2$ such that:
    \[ \frac{\sum_{i_2 \in [k_2]}\min\left(k_1,\deg_{G_W^{\mathcal{P}_2}}(\mathcal{P}_2^{i_2})\right)}{k_1\ell_2} \geq \left(1 - \frac{k_1^{k_1}e^{-k_1}}{k_1!}\right)\left(1-\left(1-\frac{1}{\ell_2}\right)^{k_2}\right)\frac{\min\left(k_1k_2,\sum_{y_1}\min(k_2,\deg_{G_W}(y_1))\right)}{k_1k_2} \ , \]
  \item Finally, we prove that:
    \[ \frac{\min\left(k_1k_2,\sum_{y_1}\min(k_2,\deg_{G_W}(y_1))\right)}{k_1k_2} \geq \mathrm{S}^{\textrm{NS}}(W,k_1,k_2) \ . \]
  \end{enumerate}
  ...
  \item This part shares a lot of similarities with the proof of Theorem~\ref{theo:DQGapprox}, which we will adapt to this particular situation. Let us show that if we take a partition $\mathcal{P}_1$ of $\mathcal{Y}_1$ of size $\ell_1$ uniformly at random, we get, for some fixed $\mathcal{P}_2$ of size $\ell_2$:
    \[ \mathbb{E}_{\mathcal{P}_1}[e_{G_W}(\mathcal{P}_1,\mathcal{P}_2)] \geq \frac{\ell_1}{k_1}\left(1-\left(1-\frac{1}{\ell_1}\right)^{k_1}\right)\sum_{i_2=1}^{\ell_2}\min\left(k_1,\deg_{G_W^{\mathcal{P}_2}}(\mathcal{P}_2^{i_2})\right) \ .\]
    ...
    \[ \mathrm{S}(W,\ell_1,\ell_2) \geq \frac{1}{\ell_1\ell_2}\mathbb{E}_{\mathcal{P}_1}[e_{G_W}(\mathcal{P}_1,\mathcal{P}_2)] \geq \left(1-\left(1-\frac{1}{\ell_1}\right)^{k_1}\right)\frac{\sum_{i_2 \in [k_2]}\min\left(k_1,\deg_{G_W^{\mathcal{P}_2}}(\mathcal{P}_2^{i_2})\right)}{k_1\ell_2} \ .\]

We have that $e_{G_W}(\mathcal{P}_1,\mathcal{P}_2)=e^1_{G_W^{\mathcal{P}_2}}(\mathcal{P}_1)=\sum_{i_2=1}^{\ell_2}e^1_{G_{W,\mathcal{P}_2^{i_2}}^{\mathcal{P}_2}}(\mathcal{P}_1)$ (with $G_W^{\mathcal{P}_2}$ being the graph quotiented only on the right part by $\mathcal{P}_2$, and $G_{W,\mathcal{P}_2^{i_2}}^{\mathcal{P}_2}$ its restriction to the right vertex $\mathcal{P}_2^{i_2}$ in the right part), so by linearity of expectation, we have that $\mathbb{E}_{\mathcal{P}_1}[e_{G_W}(\mathcal{P}_1,\mathcal{P}_2)] = \sum_{i_2=1}^{\ell_2}\mathbb{E}_{\mathcal{P}_1}[e^1_{G_{W,\mathcal{P}_2^{i_2}}^{\mathcal{P}_2}}(\mathcal{P}_1)]$, so we will focus on the contribution of one particular $\mathcal{P}_2^{i_2}$. It is enough to consider only its neighbors as the other elements of $\mathcal{Y}_1$ do not contribute in $e^1_{G_{W,\mathcal{P}_2^{i_2}}^{\mathcal{P}_2}}(\mathcal{P}_1)$.

Then, we have that $e^1_{G_{W,\mathcal{P}_2^{i_2}}^{\mathcal{P}_2}}(\mathcal{P}_1)=|\{i_1 \in [\ell_1]: N(\mathcal{P}_2^{i_2}) \cap \mathcal{P}_1^{i_1} \not= \emptyset  \}|$. Let us call $N(\mathcal{P}_2^{i_2}) = \{v_1^1, \ldots, v_1^{\deg_{G_W^{\mathcal{P}_2}}(\mathcal{P}_2^{i_2})}\}$. We have that $\mathbb{P}\left(v_1^j \in \mathcal{P}_1^{i_1}\right) = \frac{1}{\ell_1}$ since the partition is taken uniformly at random. Then, we have:

\begin{equation}
  \begin{aligned}
    \mathbb{E}_{\mathcal{P}_1}[e^1_{G_{W,\mathcal{P}_2^{i_2}}^{\mathcal{P}_2}}(\mathcal{P}_1)] &= \mathbb{E}_{\mathcal{P}_1}\left[|\{i_1 \in [\ell_1]: N(\mathcal{P}_2^{i_2}) \cap \mathcal{P}_1^{i_1} \not= \emptyset  \}|\right]
    ...
    &=\sum_{i_1 = 1}^{\ell_1}\left(1 - \prod_{j = 1}^{\deg_{G_W^{\mathcal{P}_2}}(\mathcal{P}_2^{i_2})}\left(1- \mathbb{P}\left(v_1^j \in \mathcal{P}_1^{i_1}\right)\right)\right) = \ell_1\left(1-\left(1-\frac{1}{\ell_1}\right)^{\deg_{G_W^{\mathcal{P}_2}}(\mathcal{P}_2^{i_2})}\right) \ .
  \end{aligned}
\end{equation}

So, in all we have that:
\[ \mathbb{E}_{\mathcal{P}_1}[e_{G_W}(\mathcal{P}_1,\mathcal{P}_2)] = \sum_{i_2=1}^{\ell_2}\mathbb{E}_{\mathcal{P}_1}[e^1_{G_{W,\mathcal{P}_2^{i_2}}^{\mathcal{P}_2}}(\mathcal{P}_1)] = \ell_1\sum_{i_2=1}^{\ell_2}\left(1-\left(1-\frac{1}{\ell_1}\right)^{\deg_{G_W^{\mathcal{P}_2}}(\mathcal{P}_2^{i_2})}\right) \ . \]

The function $x \mapsto 1-\left(1-\frac{1}{\ell_1}\right)^x$ is nondecreasing concave, so we have that:

\begin{equation}
  \begin{aligned}
    \mathbb{E}_{\mathcal{P}_1}[e_{G_W}(\mathcal{P}_1,\mathcal{P}_2)] &\geq \ell_1\sum_{i_2=1}^{\ell_2}\left(1-\left(1-\frac{1}{\ell_1}\right)^{\min(k_1,\deg_{G_W^{\mathcal{P}_2}}(\mathcal{P}_2^{i_2}))}\right)\\
    &\geq \ell_1\frac{\sum_{i_2=1}^{\ell_2}\min(k_1,\deg_{G_W^{\mathcal{P}_2}}(\mathcal{P}_2^{i_2}))}{k_1}\left(1-\left(1-\frac{1}{\ell_1}\right)^{k_1}\right)\\
    &= \frac{\ell_1}{k_1}\left(1-\left(1-\frac{1}{\ell_1}\right)^{k_1}\right)\sum_{i_2 \in [k_2]}\min\left(k_1,\deg_{G_W^{\mathcal{P}_2}}(\mathcal{P}_2^{i_2})\right) \ ,
  \end{aligned}
\end{equation}
since, as before, $\sum_x g(f(x)) \geq \frac{\sum_{x}{f(x)}}{M}g(M)$ if $\forall x, f(x) \leq M$ and $g$ nondecreasing concave. This concludes the first part of the proof.
  
\item Let us take $\mathcal{P}_2$ a partition of $\mathcal{Y}_2$ of size $\ell_2$ uniformly at random, and let us prove that:
  \[ \mathbb{E}\left[\sum_{i_2 \in [k_2]}\min\left(k_1,\deg_{G_W^{\mathcal{P}_2}}(\mathcal{P}_2^{i_2})\right)\right] \geq \frac{\ell_2}{k_2}\left(1 - \frac{k_1^{k_1}e^{-k_1}}{k_1!}\right)\left(1-\left(1-\frac{1}{\ell_2}\right)^{k_2}\right)\min\left(k_1k_2,\sum_{y_1}\min(k_2,\deg_{G_W}(y_1))\right) \ . \]

  First, $\sum_{i_2 \in [k_2]}\min\left(k_1,\deg_{G_W^{\mathcal{P}_2}}(\mathcal{P}_2^{i_2})\right) = \sum_{i_2=1}^{\ell_2}\varphi(\deg_{G_W^{\mathcal{P}_2}}(\mathcal{P}_2^{i_2}))$ with $\varphi(j):=\min(k_1,j)$ which is a concave function. We will use several tools introduced in~\cite{BFF21} to handle those concave functions in approximation algorithms. Specifically, we have that the Poisson concavity ratio $\alpha_{\varphi} := \inf_{x \in \mathbb{R}^+} \frac{\mathbb{E}[\varphi(\Poi(x))]}{\varphi(x))} = 1 - \frac{k_1^{k_1}e^{-k_1}}{k_1!}$ for that particular function. We will also need the following lemma, whose proof is based on convex order:
...
Let us find the law of $\deg_{G_W^{\mathcal{P}_2}}(\mathcal{P}_2^{i_2})$:
  \begin{equation}
    \begin{aligned}
      \deg_{G_W^{\mathcal{P}_2}}(\mathcal{P}_2^{i_2}) &= \sum_{y_1} \mathbbm{1}_{N(y_1) \cap \mathcal{P}_2^{i_2} \not= \emptyset} = \sum_{y_1} \left(1-\mathbbm{1}_{N(y_1) \cap \mathcal{P}_2^{i_2} = \emptyset}\right) = \sum_{y_1} \left(1-\mathbbm{1}_{\forall y_2 \in N(y_1), y_2 \not\in \mathcal{P}_2^{i_2}}\right)\\
      &= \sum_{y_1} \Ber\left(1-\left(1-\frac{1}{\ell_2}\right)^{\deg_{G_W}(y_1)}\right)
    \end{aligned}
  \end{equation}
...
\begin{equation}
  \begin{aligned}
    \mathbb{E}\left[\varphi(\deg_{G_W^{\mathcal{P}_2}}(\mathcal{P}_2^{i_2}))\right] &= \mathbb{E}\left[\varphi\left(\sum_{y_1}\Ber\left(1-\left(1-\frac{1}{\ell_2}\right)^{\deg_{G_W}(y_1)}\right)\right)\right]\\
    &\geq  \mathbb{E}\left[\varphi\left(\Poi\left(\sum_{y_1}\left(1-\left(1-\frac{1}{\ell_2}\right)^{\deg_{G_W}(y_1)}\right)\right)\right)\right] \text{ by Lemma~\ref{lem:ConvexOrder}}\\
    &\geq \alpha_{\varphi}\varphi\left(\sum_{y_1}\left(1-\left(1-\frac{1}{\ell_2}\right)^{\deg_{G_W}(y_1)}\right)\right) \text{ by definition of $\alpha_{\varphi}$.}
  \end{aligned}
\end{equation}
...
\begin{equation}
  \begin{aligned}
    \sum_{y_1}\left(1-\left(1-\frac{1}{\ell_2}\right)^{\deg_{G_W}(y_1)}\right) &\geq \sum_{y_1}\left(1-\left(1-\frac{1}{\ell_2}\right)^{\min\left(k_2,\deg_{G_W}(y_1)\right))}\right)\\
    &\geq \left(1-\left(1-\frac{1}{\ell_2}\right)^{k_2}\right)\frac{1}{k_2}\sum_{y_1}\min\left(k_2,\deg_{G_W}(y_1)\right) \ ,
  \end{aligned}
\end{equation}

as before. $\varphi$ is also sublinear, so:

\begin{equation}
  \begin{aligned}
    \mathbb{E}\left[\varphi(\deg_{G_W^{\mathcal{P}_2}}(\mathcal{P}_2^{i_2})))\right] &\geq \alpha_{\varphi}\left(1-\left(1-\frac{1}{\ell_2}\right)^{k_2}\right)\min\left(k_1,\frac{1}{k_2}\sum_{y_1}\min\left(k_2,\deg_{G_W}(y_1)\right)\right)\\
    &= \frac{1}{k_2}\left(1 - \frac{k_1^{k_1}e^{-k_1}}{k_1!}\right)\left(1-\left(1-\frac{1}{\ell_2}\right)^{k_2}\right)\min\left(k_1k_2,\sum_{y_1}\min\left(k_2,\deg_{G_W}(y_1)\right)\right)\\
  \end{aligned}
\end{equation}

since $\alpha_{\varphi} = 1 - \frac{k_1^{k_1}e^{-k_1}}{k_1!}$.

Finally, $\mathbb{E}\left[\sum_{i_2 \in [k_2]}\min\left(k_1,\deg_{G_W^{\mathcal{P}_2}}(\mathcal{P}_2^{i_2})\right)\right] = \sum_{i_2=1}^{\ell_2}\mathbb{E}\left[\varphi(\deg_{G_W^{\mathcal{P}_2}}(\mathcal{P}_2^{i_2}))\right]$, so we get that:
\[ \mathbb{E}\left[\sum_{i_2 \in [k_2]}\min\left(k_1,\deg_{G_W^{\mathcal{P}_2}}(\mathcal{P}_2^{i_2})\right)\right] \]
is larger than:
\[ \frac{\ell_2}{k_2}\left(1 - \frac{k_1^{k_1}e^{-k_1}}{k_1!}\right)\left(1-\left(1-\frac{1}{\ell_2}\right)^{k_2}\right)\min\left(k_1k_2,\sum_{y_1}\min\left(k_2,\deg_{G_W}(y_1)\right)\right) \ . \]
...
\item Let us consider an optimal solution $r_{x,y_1,y_2},p_x,r^1_{x,y_1},r^2_{x,y_2}$ of the program computing $\mathrm{S}^{\textrm{NS}}(W,k_1,k_2)$, so that $\mathrm{S}^{\textrm{NS}}(W,k_1,k_2) = \frac{1}{k_1k_2}\sum_x r_{x,W_1(x),W_2(x)}$.
  ...
  \item $\sum_x r_{x,W_1(x),W_2(x)} = \sum_{y_1}\sum_{x:W_1(x)=y_1} r_{x,y_1,W_2(x)}$ and we have that:
    \begin{enumerate}
    \item $\sum_{x:W_1(x)=y_1} r_{x,y_1,W_2(x)} \leq \sum_{x:W_1(x)=y_1} 1 = \deg_{G_W}(y_1)$ ,
    \item $\sum_{x:W_1(x)=y_1} r_{x,y_1,W_2(x)} \leq \sum_{x:W_1(x)=y_1} r^1_{x,y_1} \leq \sum_x r^1_{x,y_1} = k_2$ ,
    \end{enumerate}
    so $\sum_{x:W_1(x)=y_1} r_{x,y_1,W_2(x)} \leq \min(k_2,\deg_{G_W}(y_1))$, and thus:
    \[ \sum_x r_{x,W_1(x),W_2(x)} \leq \sum_{y_1}\min(k_2,\deg_{G_W}(y_1)) \ .\]
  \end{enumerate}

  In all, we get that:
  \[ \mathrm{S}^{\textrm{NS}}(W,k_1,k_2) = \frac{1}{k_1k_2}\sum_x r_{x,W_1(x),W_2(x)} \leq \frac{\min\left(k_1k_2,\sum_{y_1}\min(k_2,\deg_{G_W}(y_1))\right)}{k_1k_2} \ , \]
...
The goal of this section is to show that the broadcast channel coding problem in the general case cannot be approximated in polynomial time within a $\Omega(1)$ factor. It will be a good insight that non-signaling assistance will enlarge the capacity region of the channel.

Indeed, if capacity regions with and without non-signaling assistance were the same, then, under some meta-converse hypothesis, there would exist some constant (independent from $k_1, k_2$ and $n$) multiplicative factor between the objective value of the program with and without non-signaling assistance. Therefore, this would imply that computing the non-signaling assisted value would give a constant factor approximation of the unassisted problem. Note that computing the value of non-signaling assisted program can be done in polynomial time as it is a linear program.

So, in all, if the non-signaling assisted capacity region is the same as the unassisted capacity region, with some meta-converse theorem, then there would exist a $\Omega(1)$ approximation algorithm running in polynomial time to the unassisted problem. Thus, showing that no such approximation exists implies that there is a gap between the non-signaling assisted capacity region and the unassisted one.

However, showing unconditional results of the sort is out of reach. A natural direction would be to show that obtaining such approximation factor in polynomial time is \textrm{NP}-hard. This objective seems however intractable. Hence, we will prove a more constrained hardness of approximation, under the value-query model.
...
Let us show that computing $\mathrm{S}(W,k_1,|\mathcal{Y}_2|)$ can be reformulated as a particular instance of the social welfare maximization problem. First, we can write:

\begin{equation}
  \begin{aligned}
    \mathrm{S}(W,k_1,k_2) = && \underset{e,d_1,d_2}{\maxi} &&& \frac{1}{k_1k_2}\sum_{y_1,y_2,i_1,i_2} W(y_1y_2|e(i_1,i_2))\mathbbm{1}_{d_1(y_1)=i_1, d_2(y_2)=i_2} \ ,
  \end{aligned}
\end{equation}

with $e : [k_1] \times [k_2] \rightarrow \mathcal{X}$, $d_1 : \mathcal{Y}_1 \rightarrow [k_1]$,  $d_2 : \mathcal{Y}_2 \rightarrow [k_2]$. Indeed, this comes from the fact that a linear program is maximized on its extremal points. Then:

\begin{equation}
  \begin{aligned}
    \mathrm{S}(W,k_1,k_2) = && \underset{e,d_1,d_2}{\maxi} &&& \frac{1}{k_1k_2}\sum_{i_1,i_2} \sum_{y_1 \in d_1^{-1}(i_1), y_2 \in d_2^{-1}(i_2)} W(y_1y_2|e(i_1,i_2)) \\
    = && \underset{\mathcal{P}_1,\mathcal{P}_2}{\maxi} &&& \frac{1}{k_1k_2}\sum_{i_1,i_2} \max_{e(i_1,i_2)} \sum_{y_1 \in \mathcal{P}_1^{i_1}, y_2 \in \mathcal{P}_1^{i_2}} W(y_1y_2|e(i_1,i_2)) \\
    &&&&&\text{with $\mathcal{P}_1,\mathcal{P}_2$ partitions of $\mathcal{Y}_1, \mathcal{Y}_2$ respectively} \\
...
Thus, in all, we have that:

\begin{equation}
  \begin{aligned}
    \mathrm{S}(W,k_1,k_2) = && \underset{\mathcal{P}_1,\mathcal{P}_2}{\maxi} &&& \frac{1}{k_1k_2}\sum_{i_1,i_2} f_W(\mathcal{P}_1^{i_1}, \mathcal{P}_1^{i_2}) \text{ with } f_W(S_1,S_2) := \max_x \sum_{y_1 \in S_1, y_2 \in S_2} W(y_1y_2|x) \ .
  \end{aligned}
\end{equation}

We will now focus on the particular case of $k_2=|\mathcal{Y}_2|$. In that case, it is easy to see that $\mathcal{P}_2 = (\{y_2\})_{y_2 \in \mathcal{Y}_2}$ is always an optimal solution. Indeed, for any partition $\mathcal{P}_2$, we have:

\begin{equation}
  \begin{aligned}
    \frac{1}{k_1k_2}\sum_{i_1,i_2}  \max_x \sum_{y_1 \in \mathcal{P}_1^{i_1}, y_2 \in \mathcal{P}_2^{i_2}} W(y_1y_2|x) &\leq \frac{1}{k_1k_2}\sum_{i_1,i_2}  \sum_{y_2 \in \mathcal{P}_2^{i_2}} \max_x \sum_{y_1 \in \mathcal{P}_1^{i_1}} W(y_1y_2|x) \\
    &= \frac{1}{k_1k_2}\sum_{i_1}  \sum_{y_2 \in \mathcal{Y}_2} \max_x \sum_{y_1 \in \mathcal{P}_1^{i_1}} W(y_1y_2|x) \ .
  \end{aligned}
\end{equation}

We can now rewrite our program in the following way:

\begin{equation}
  \begin{aligned}
    \mathrm{S}^1(W,k_1) := k_1k_2\mathrm{S}(W,k_1,|\mathcal{Y}_2|) = && \underset{\mathcal{P}_1}{\maxi} &&& \sum_{i_1} \sum_{y_2} \max_x \sum_{y_1 \in \mathcal{P}_1^{i_1}} W(y_1y_2|x) \ .
  \end{aligned}
\end{equation}
...
\begin{equation}
  \begin{aligned}
    \mathrm{S}^1(W,k_1) = && \underset{\mathcal{P}_1}{\maxi} &&& \sum_{i_1} f_W^1(\mathcal{P}_1^{i_1}) \text{ with } f_W^1(S_1) := \sum_{y_2} \max_x \sum_{y_1 \in S_1} W(y_1y_2|x)
  \end{aligned}
\end{equation}


Hence, computing $\mathrm{S}^1(W,k_1)$ is a particular case of the social welfare maximization problem with a common utility $f_W^1$ for all $k_1$ bidders.
...
\begin{proof}
  \begin{equation}
    \begin{aligned}
      &&f_W^1(S) &= \sum_{y_2} \max_x \sum_{y_1 \in S_1} W(y_1y_2|x) = \max_{\lambda : \mathcal{Y}_2 \rightarrow \mathcal{X}} a_{\lambda}(S) \text{ , where}\\
      &&a_{\lambda}(S) &= \sum_{y_2}  \sum_{y_1 \in S} W(y_1y_2|\lambda(y_2)) = \sum_{y_1 \in S} \left[\sum_{y_2}  W(y_1y_2|m(y_2))\right]\\
      &&&= \sum_{y_1 \in S} a_{\lambda}(\{ y_1 \})) \text{ with } a_{\lambda}(\{ y_1 \})) = \sum_{y_2}  W(y_1y_2|\lambda(y_2)) \in \mathbb{R}_+
      \end{aligned}
  \end{equation}
...
\begin{theo}
  ...
  In the value query model, for any fixed $\varepsilon > 0$, a $\Omega\left(\frac{1}{m^{\frac{1}{2}-\varepsilon}}\right)$-approximation algorithm computing $\mathrm{S}^1(W,k_1)=k_1k_2\mathrm{S}(W,k_1,|\mathcal{Y}_2|)$, with $m = |\mathcal{Y}_1|$ and $W$ a broadcast channel, requires exponentially many value queries to $f_W^1$.
\end{theo}
...
\begin{proof}
  ...
  Let us fix a small constant $\delta > 0$. We choose $k \in \mathbb{N}$ bidders for $m := k^2$ items, and thus $\mathcal{Y}_1 := [m]$. Then, we choose uniformly at random an equi-partition of $\mathcal{Y}_1$ in $k$ parts of size $k$, which we name $T_1, \ldots, T_k$.

  Let us define now $\mathcal{Y}_2 := [m+k+1]$. We take $\mathcal{X} := \mathcal{Y}_2 = [m+1+k]$ as well. We can now define our broadcast channel $W$, with some positive constant $C$ to be fixed later to guarantee that $W$ is a conditional probability distribution. Let us define its value for $y_2=1$:
  \[ W(y_1 1|x) := C\times\begin{cases}
    m^{2\delta}\mathbbm{1}_{y_1=x} & \text{when } 1 \leq x \leq m\\
  \frac{1}{m^{\frac{1}{2}-\delta}} & \text{when } x = m+1\\
  \mathbbm{1}_{y_1 \in T_j} & \text{when } 1 \leq j := x-(m+1) \leq k\\
  \end{cases}
  \]

  Then, we define other $y_2$ inputs as translations of $W(y_1 1|x)$. Specifically, we define:
  \[ W(y_1y_2|x) := W(y_11|t_{y_2-1}(x)) \text{ with } t_s(x) := 1 + [(x-1+s) \mod (m+k+1)] \ .\] 
  ...
  \begin{equation}
    \begin{aligned}
      ...
      &= C \sum_{y_1}\left[\sum_{1 \leq i \leq m} m^{2\delta}\mathbbm{1}_{y_1=i} + \frac{1}{m^{\frac{1}{2}-\delta}} + \sum_{1 \leq j \leq k} \mathbbm{1}_{y_1 \in T_j}\right]\\
      &= C \left[\sum_{1 \leq i \leq m} m^{2\delta} + m\times\frac{1}{m^{\frac{1}{2}-\delta}} + \sum_{1 \leq j \leq k} k\right]\\
  ...
  \begin{equation}
    \begin{aligned}
      f_W^1(S) &= \sum_{y_2} \max_x \sum_{y_1 \in S} W(y_1y_2|x) = \sum_{y_2} \max_x \sum_{y_1 \in S} W(y_11|t_{y_2-1}(x))\\
      &= (m+k+1)\max_x \sum_{y_1 \in S} W(y_11|x) \text{ since $t_{y_2-1}$ bijection}\\
      &= C(m+k+1) \times \max\begin{cases}
      m^{2\delta}|\{i\} \cap S| & \text{for } 1 \leq i \leq m\\
      \frac{1}{m^{\frac{1}{2}-\delta}}|S|\\
      |T_j \cap S| & \text{for } 1 \leq j \leq k\\
      \end{cases}
    \end{aligned}
  \end{equation}
      
  Let us also consider an alternate broadcast channel $W'$, with the only difference that $\mathbbm{1}_{y_1 \in T_j}$ is replaced by $\frac{1}{m^{\frac{1}{2}}}$, for $j \in [k]$. For that channel, the constant $C$ remains the same (since $\sum_j\sum_{y_1}\mathbbm{1}_{y_1 \in T_j} = k \times k = k \times m \times \frac{1}{k} = \sum_j\sum_{y_1}\frac{1}{m^{\frac{1}{2}}}$), so we get that:
  \begin{equation}
    \begin{aligned}
      f_{W'}^1(S) &= C(m+k+1) \times \max\begin{cases}
      m^{2\delta}|\{i\} \cap S| & \text{for } 1 \leq i \leq m\\
      \frac{1}{m^{\frac{1}{2}-\delta}}|S|\\
      \frac{1}{m^{\frac{1}{2}}}|S| & \text{for } 1 \leq j \leq k\\
      \end{cases}\\
      &= C(m+k+1) \times \max\begin{cases}
      m^{2\delta}|\{i\} \cap S| & \text{for } 1 \leq i \leq m\\
      \frac{1}{m^{\frac{1}{2}-\delta}}|S|\\
  ...
  since $\frac{1}{m^{\frac{1}{2}}}|S| \leq \frac{1}{m^{\frac{1}{2}-\delta}}|S|$. We will prove that it takes an exponential number of value queries to distinguish between $f_W^1$ and $f_{W'}^1$. On the one hand, one can easily show that the maximum value of $\frac{\mathrm{S}^1(W',k_1)}{C(m+k+1)}$ is $(k-1)m^{2\delta}+\frac{1}{m^{\frac{1}{2}-\delta}}(m-(k-1)) = O(m^{\frac{1}{2}+2\delta})$, obtained taking $(k-1)$ singletons for the first bidders, giving the rest of the items to the last. On the other hand, the maximum value of $\frac{\mathrm{S}^1(W,k_1)}{C(m+k+1)}$ is $k \times k = m$, obtained with bidders choosing items accordingly to the partition $T_1, \ldots, T_k$. The fact that it requires an exponential number of value queries to distinguish between the two situations will imply that one cannot get an approximation rate better than $\Omega\left(\frac{1}{m^{\frac{1}{2}-2\delta}}\right)$ in less than an exponential number of value queries.

  We will now prove that distinguishing between $f_W^1$ and $f_{W'}^1$ requires an exponential number of value queries. Let us focus on one bidder. We define $v(S) := \frac{f_W^1(S)}{C(m+k+1)}$ and $v'(S) := \frac{f_{W'}^1(S)}{C(m+k+1)}$, so distinguishing between $v$ and $v'$ is the same as distinguishing between $f_W^1$ and $f_{W'}^1$. We have always $v(\emptyset) = v'(\emptyset) = 0$, so we do not need to consider empty sets.

  Let us fix some non-empty set $S \subseteq [m]$. Let us define the random boolean variables $X_j^i := \mathbbm{1}_{i \in T_j}$ for $j \in [k]$ and $i \in [m]$. By construction of the random equi-partition $T_1, \ldots, T_k$, $(X_j^i)_{i \in [m]}$ is a permutation distribution (see Definition 2.10 of~\cite{JP83}) of $(0,\ldots,0,1,\ldots,1)$ with $m-k$ zeros and $k$ ones, each $X_j^i$ following a Bernouilli law of parameter $p := \frac{1}{k}$. Thus it is negatively associated (by Theorem 2.11 of~\cite{JP83}), and the sub-family $(X_j^i)_{i \in S}$ is negatively associated as well. Note in particular that $|T_j \cap S| = \sum_{i \in S} X_j^i$ is a sum of negatively associated Bernouilli variables of the same parameter $p$, so Chernoff-Hoeffding bound as depicted in Proposition~\ref{prop:chernoff} holds.
  ...
  Let us first assume that $S$ is of size $0 < |S| \leq m^{\frac{1}{2}+\delta}$. Then, we have that $\frac{1}{m^{\frac{1}{2}-\delta}}|S| \leq m^{2\delta}$, so we get that $v'(S) = m^{2\delta}$. On the other hand, we have that:
    ...
    Thus, $v(S)$ is different from $v'(S)$ if and only if $\exists j \in k, |T_j \cap S| > m^{2\delta}$. But, we have:
    \begin{equation}
      \begin{aligned}
        &\mathbb{P}\left(\exists j \in k, |T_j \cap S| > m^{2\delta}\right) \leq \sum_{j \in [k]} \mathbb{P}\left(|T_j \cap S| > m^{2\delta}\right) \text{ by union bound}\\
        &=\sum_{j \in [k]} \mathbb{P}\left(\sum_{i \in S}X_j^i > m^{2\delta}\right)
        =\sum_{j \in [k]} \mathbb{P}\left(\frac{1}{|S|}\sum_{i \in S}X_j^i > \left(1 + \frac{\frac{m^{2\delta}}{[S|}-1}{p}\right)p \right)\\
        &\leq \sum_{j \in [k]} \mathbb{P}\left(\frac{1}{|S|}\sum_{i \in S}X_j^i > \left(1 + \frac{\frac{m^{2\delta}}{|S|}}{p}\right)p \right)\\
        &\leq \sum_{j \in [k]}\exp\left(-\frac{p|S|}{4}\left(\frac{m^{2\delta}}{p|S|}\right)^2\right) \text{ by Chernoff-Hoeffding bound as depicted in Proposition~\ref{prop:chernoff}}\\
        &= \sum_{j \in [k]}\exp\left(-\frac{1}{4p|S|}m^{4\delta}\right) \leq \sum_{j \in [k]}\exp\left(-\frac{m^{-\delta}}{4}m^{4\delta}\right) \text{ since } \frac{1}{p|S|} = \frac{k}{|S|} \geq m^{-\delta}\\
        &= m^{\frac{1}{2}}e^{-\frac{m^{3\delta}}{4}}\ .
      \end{aligned}
    \end{equation}

    Thus, this event occurs with exponentially small probability (on the choice of the partition $T_1,\ldots,T_k$).
    ...
    Thus, $v(S)$ is different from $v'(S)$ if and only if $\exists j \in [k], |T_j \cap S| > \frac{1}{m^{\frac{1}{2}-\delta}}|S|$. But, we have:
        \begin{equation}
      \begin{aligned}
        &\mathbb{P}\left(\exists j \in [k], |T_j \cap S| > \frac{1}{m^{\frac{1}{2}-\delta}}|S|\right) \leq \sum_{j \in [k]} \mathbb{P}\left(|T_j \cap S| > \frac{1}{m^{\frac{1}{2}-\delta}}|S|\right) \text{ by union bound}\\
        &=\sum_{j \in [k]} \mathbb{P}\left(\sum_{i \in S}X_j^i > \frac{1}{m^{\frac{1}{2}-\delta}}|S|\right)
        =\sum_{j \in [k]} \mathbb{P}\left(\frac{1}{|S|}\sum_{i \in S}X_j^i > \left(1 + \frac{\frac{1}{|S|m^{\frac{1}{2}-\delta}}{|S|}-1}{p}\right)p \right)\\
        &\leq \sum_{j \in [k]} \mathbb{P}\left(\frac{1}{|S|}\sum_{i \in S}X_j^i > \left(1 + \frac{\frac{1}{m^{\frac{1}{2}-\delta}}}{p}\right)p \right)\\
        &= \sum_{j \in [k]} \mathbb{P}\left(\frac{1}{|S|}\sum_{i \in S}X_j^i > \left(1 + m^{\delta}\right)p \right) \text{ since } p = \frac{1}{k} = \frac{1}{m^{\frac{1}{2}}}\\
        &\leq \sum_{j \in [k]}\exp\left(-\frac{p|S|}{4}m^{2\delta}\right) \text{ by Chernoff-Hoeffding bound as depicted in Proposition~\ref{prop:chernoff}}\\
        &\leq \sum_{j \in [k]}\exp\left(-\frac{m^{\delta}}{4}m^{2\delta}\right) \text{ since } p|S| = \frac{|S|}{k} \geq m^{\delta}\\
        &= m^{\frac{1}{2}}e^{-\frac{m^{3\delta}}{4}}\ .
      \end{aligned}
    \end{equation}
    ...
    Hence, for every set $S$, only with exponentially small probability $p_{\text{leak}}$ can one distinguish between $v$ and $v'$. For some fixed algorithm $\mathcal{A}$, let us consider the sequence $L$ of queries made by $\mathcal{A}$ before it is able to distinguish between $v$ and $v'$: $L := (S_1, \ldots, S_n)$, with $v(S_i) = v'(S_i)$ for $i \in [n]$ and $v'(S_{n+1}) > v(S_{n+1})$. $L$ is independent from $T_1, \ldots T_k$ as no information from this partition is leaked before $S_{n+1}$. Thus, for such an algorithm to be correct, it should work for any equi-partition $T_1, \ldots T_k$. We have:
...
Our results suggest that non-signaling assistance could improve the capacity region of general broadcast channels, which is left as a major open question. An intermediate result would be to show that it is \textrm{NP}-hard to approximate the broadcast channel coding problem within any constant, strengthening our hardness result without relying on the value query model. Finally, one could also try to find approximations algorithms for other sub-classes of broadcast channels, such as semi-deterministic or degraded ones. This could be a crucial step towards showing that the capacity region for those classes is the same with or without non-signaling assistance.

\section*{Acknowledgements}
We would like to thank Stéphan Thomassé and Siddharth Barman for discussions respectively on graph algorithms and social welfare problems.
