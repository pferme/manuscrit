In this chapter, we recall briefly basic definitions and properties, as well as more unusual results, used throughout the thesis. Rather than an introduction to each of those topics, one should see this chapter as a toolbox and use it when needed in the other chapters.

\section{Probability Theory}
\addcontentsline{tof}{section}{\protect\numberline{\thesection} Probabilités}
...
\begin{proposition}[Jensen's Inequality~\cite{Jensen1906}]
...
\begin{proposition}[Property $\text{P}_1$ of~\cite{JP83}]
  \label{prop:NA_P1}
...
\begin{proposition}[Property $\text{P}_7$ of~\cite{JP83}]
    \label{prop:NA_P7}
...
\begin{proposition}[Theorem 2.11 of~\cite{JP83}]
  \label{prop:permNA}
   A permutation distribution is negatively associated.
\end{proposition} 

\begin{proposition}[Chernoff-Hoeffding bound - version 1]
...
\begin{proposition}[Chernoff-Hoeffding bound - version 2]
...
\addcontentsline{tof}{section}{\protect\numberline{\thesection} Théorie de l'information}
...
We say that $X \rightarrow Y \rightarrow Z$ forms a Markov chain if $Z$ is conditionally independent of $X$ given $Y$, that is to say:
    ...
    \item $\forall y^n \in \mathcal{T}^n_{\varepsilon}(Y), \mathcal{T}^n_{\varepsilon}(X|y^n) := \{ x^n : (x^n,y^n) \in \mathcal{T}^n_{\varepsilon}(X,Y) \}$.
      ...
      \begin{proof}
        ...
        Thus, all $Z_i$ with $i \in S$ are independent and follow the same law:      
          ...
          Thus, we have $\frac{\pi(x,y|X^n,y^n)}{\pi(y|y^n)} \underset{n \rightarrow +\infty}{\rightarrow} P_{X|Y}(x|y)$ in probability with a convergence rate independent of $y^n \in \mathcal{T}^n_{\varepsilon'}(Y)$.
      \end{proof}
...
When $n=m=1$, we will speak of regular or point-to-point channels. When $n>1,m=1$, we will speak of multiple-access channels. When $n=1,m>1$, we will speak of broadcast channels. If both $n,m$ are greater than $1$, we will speak of interference channels. More specifically, in this thesis, we will focus on the cases of $n=2,m=1$ and $n=1,m=2$, which are at the core of the specificity of network channels.
...
\begin{definition}[Zero-error Capacity Region ${\mathcal{C}_0[\mathrm{S}](W)}$ for $\mathrm{S}(W,k_1,k_2)$]
...
\addcontentsline{tof}{section}{\protect\numberline{\thesection} Problème du codage pour les canaux un-à-un}
