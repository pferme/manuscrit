...
Negatively associated random variables definition, recall important properties etc. (clean 3.5.4 with and paragraph before (5.44) with that)

\subsection{Tail Bounds}
\begin{proposition}[Chernoff-Hoeffding bound - version 1]
  \label{prop:chernoff1}
  Let $X_1, \ldots, X_m$ be negatively associated random variables such that $0 \leq X_i \leq B$ and $\mathbb{E}[X_i] = \mu$. Then for $\eta \in (0,1)$:
  \[ \mathbb{P}\left(\left|\frac{1}{m}\sum_{i=1}^m X_i - \mu\right| > \eta \right) \leq 2e^{-2\left(\frac{\eta}{B}\right)^2m \ . \]
\end{proposition}
\begin{proof}
