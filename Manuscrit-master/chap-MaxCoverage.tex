...
We can directly reduce  the standard  maximum coverage problem to $\varphi$-\textsc{MaxCoverage} by setting $\varphi(j) = \min\{ j,1\}$. In this case $\alpha_{\varphi} = 1 - e^{-1}$. One can also encapsulate, within our framework, the $\ell$-\textsc{MultiCoverage} problem studied in~\cite{BFGG20} by instantiating $\varphi(j) = \min\{ j,\ell\}$. In this setting, we recover the approximation ratio $\alpha_{\varphi} = 1-\frac{\ell^{\ell}e^{-\ell}}{\ell!}$, which matches the approximation guarantee obtained in~\cite{BFGG20} (see Proposition \ref{prop:lCover}). Note that the hardness result in~\cite{BFGG20} was based on the Unique Games Conjecture, whereas the current work proves that this guarantee is tight under $\textrm{P} \not= \textrm{NP}$.
