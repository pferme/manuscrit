...
\begin{rk}
A PTAS does not necessarily run in polynomial time in $\frac{1}{\varepsilon}$.
\end{rk}

\begin{proof}
  Let us take $\varepsilon := \frac{1-\alpha_{\varphi}}{2}\varphi(x_{\varphi}) > 0$. In the previous proof, with such an additive approximation algorithm, one could distinguish between YES and NO instance of $\delta,h$-\textsc{AryGapLabelCover}. Indeed, a YES instance implies that the associated $\varphi$-\textsc{MaxCoverage} instance has a solution of value $\varphi(x_{\varphi}) \cdot |\Gamma|$, where $\Gamma$ is the universe in that setting. On the other hand, a NO instance implies that all the solutions of the associated $\varphi$-\textsc{MaxCoverage} instance have a value smaller than or equal to $\alpha_\varphi\varphi(x_{\varphi}) \cdot |\Gamma|$. Therefore, on a YES instance, our approximation algorithm would output a solution with a value larger than or equal to $\frac{1+\alpha_{\varphi}}{2}\varphi(x_{\varphi}) \cdot |\Gamma|$. This resulting inequality is enough to guarantee that it does not come from a NO instance, as it is strictly larger than $\alpha_\varphi\varphi(x_{\varphi}) \cdot |\Gamma|$. Thus, such an approximation algorithm would allow us to solve $\delta,h$-\textsc{AryGapLabelCover} in polynomial time, which cannot happen if \textrm{P}$\not=$\textrm{NP}.
\end{proof}
