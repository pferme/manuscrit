...
\begin{frame}{Proof ideas: approximation algorithm}
  \begin{itemize}
  \item \emph{Relax and round} strategy with \emph{pipage rounding}~\cite{AS04, Vondrak07}.
    \bigskip
    \pause
  \item Linear relaxation using piecewise linear extention of $\varphi$ on $\mathbb{R}^+$.
  \item $C^{\varphi}$ submodular: pipage rounding on its multilinear extension. Finds in polynomial time an integral solution $x^{\text{int}}$ such that $C^{\varphi}(x^{\text{int}}) \geq \mathbb{E}_{X \sim \Ber(x^*)}[C^{\varphi}(X)]$ given the optimal fractional solution $x^*$.
  \item Approximation guarantee theorem links previous expectation to the maximum value of the relaxed program.
  \end{itemize}

  \pause
  \begin{align*}
    \hspace*{-0.7cm} C^{\varphi}(x^{\text{int}}) \underset{\text{Rounding}}{\overset{\text{Pipage}}{\geq}} \mathbb{E}_{X \sim \Ber(x^*)}[C^{\varphi}(X)]  \overset{\text{AGT}}{\geq} \alpha_{\varphi} \max_x  C^{\varphi}(x) \underset{\text{Relax}}{\geq} \alpha_{\varphi} \max_{\abs{S} = k} C^{\varphi}(S)\ .
  \end{align*}
\end{frame}
  ...
  \begin{center}
      \begin{tikzpicture}[scale=0.75]
        ...
        \only<3->{\addplot[
         ...
         \legend{,,,Classical CR, Best ZE Lower Bounds, \only<3->{NS ZE CR  for $2$ copies}, \only<4->{NS ZE CR  for $3$ copies}, \only<5->{NS ZE CR for $7$ copies}}
        \end{axis}
      \end{tikzpicture}
    \end{center}
\end{frame}
