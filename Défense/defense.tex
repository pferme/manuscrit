...
\begin{frame}{Communiquer avec du bruit ?}
  ...
  \begin{tikzpicture}
    ...
    \only<2>{\node[dashed, ellipse callout, draw, callout relative pointer={(-0.2,0)}] at (9.7,0.1) (Bob) {\tiny À l'eau!};}
...
\begin{frame}{Modélisation mathématique}
  \begin{center}
    \begin{tikzpicture}[auto, node distance=2cm,>=latex']
      ...
      \node [right of=W] (y) {\textcolor{blue}{\only<1-2>{$y$}\only<3>{\emph{alfa}}\only<4>{\emph{alma}}\only<5>{\emph{alvo}}}};
      \node [left of=W] (x) {\textcolor{blue}{\only<1-2>{$x$}\only<3-5>{\emph{alfa}}}};
      \draw [->] (x) -- (W);
      \draw [->] (W) -- (y);
    \end{tikzpicture}
    ...
    \pause
    \bigskip
    Probabilité \only<2>{$W(y|x)$}\only<3>{$6/10$}\only<4>{$3/10$}\only<5>{$1/10$} d'avoir la sortie \textcolor{blue}{\only<2>{$y$}\only<3>{\emph{alfa}}\only<4>{\emph{alma}}\only<5>{\emph{alvo}}} pour l'entrée \textcolor{blue}{\only<2>{$x$}\only<3-5>{\emph{alfa}}}
  \end{center} 
\end{frame}
...
\begin{frame}{Le problème du codage de canal}
  \begin{center}
    ...
    Trouver $e,d$ qui maximise la probabilité d'avoir \alert{$j=i$}\only<5>{\ldots\\
      \bigskip
...
\begin{frame}{Décodage de liste}
  \begin{center}
    ...
    \bigskip
    Trouver $e,d$ qui maximise la probabilité que \only<1-2>{\alert{$j_1$ ou $j_2$}}\only<3-6>{\alert{$j_1,j_2,\ldots$ ou $j_{\ell}$}} $=$ \alert{$i$}.
  \end{center}
...
\begin{frame}{De quoi parle la physique quantique ?}
  ...
  \item Comprendre physique à l'\emph{échelle} des atomes -- \emph{un million de fois plus petit qu'un grain de sable !} (0,1nm)
    ...
    \end{center}
    \pause
  \item Phénomène à cette échelle...\pause \textcolor{blue}{l'intrication quantique :}
    \pause
    \\ \ 
    \begin{center}
...
\begin{frame}{Le paradoxe Einstein-Podolski-Rosen}
  ...
  \item \underline{Paradoxe EPR~\cite{EPR35}:} deux particules intriquées à distance
    ...
    \end{enumerate}
    \pause
  \item \alert{Le premier !} Réalité contredit (en partie) \emph{principe de localité}.
    \pause
  \item Communication plus rapide que la lumière ? \textcolor{blue}{\emph{Non, corrélations !}}
  \end{itemize}
\end{frame}
...
\begin{frame}{Un exemple d'inégalité de Bell : le jeu CHSH}
  \begin{center}
   ...
   \begin{tikzpicture}[auto, node distance=1.8cm,>=latex']
      ...
      \only<7->{\node [starburst, fill=white, draw=darkgreen, line width=1pt, text=darkgreen] at (I) {\Large \only<7>{$3/4$}\only<8>{$75\%$}};}
   \end{tikzpicture}
   \bigskip
   \\
...
\begin{frame}{Variables cachées, intrication quantique et non-signalante}
  \begin{center}
    ...
    \begin{tikzpicture}[auto, node distance=1.8cm,>=latex']
      ...
      \only<4>{\node [starburst, fill=white, draw=darkgreen, line width=1pt, text=darkgreen] at (I) {\Large $75\%$};}
...
\begin{frame}{Assistance non-signalante pour le codage de canal}
  \begin{center}
    \begin{tikzpicture}[auto, node distance=1.8cm,>=latex']
      ...
      \only<2->{\node [block] (A) {\only<2>{$e$}\only<3->{\includegraphics[scale=0.08]{Alice.png}}};
      \node [above of=A] (x) {$i$};
      \draw [->] (x) -- (A);
      ...
      \node [block, right of=I] (B) {\only<2>{$d$}\only<3->{\includegraphics[scale=0.16]{Bob.jpg}}};
      \node [above of=B] (y) {$y$};
      \draw [->] (y) -- (B);
      \node [below of=B] (b) {\only<2>{$j$}\only<3->{$\alert{j}$}};
      \draw [->] (B) -- (b);

      \only<2->{\node [right of=B] (Bright) {};
      \node [bigblock,right of=Bright] (W) {$W$};
      \draw [->] (a) to[out=-20,in=-90] (W);
      \draw [->] (W) to[out=90,in=20] (y);}}
      ...
      \node [color=blue,below of=I,outer sep=0pt,circle,fill,inner sep=1pt] (Ibelow) {};
      \draw [color=blue,dotted, thick] (Iabove) -- (Ibelow);}
      \only<5->{\draw [snake=coil,segment aspect=0, draw=blue] (A) -- (B);
        \node [circle, fill=white, draw=blue, text=blue] at (I) (Corr) {\textrm{NS}};}
    \end{tikzpicture}\\
    \only<5->{\underline{\cite{BF18} :}  meilleure communication (jusqu'à $58\%$ ! ($\frac{e^{-1}}{1-e^{-1}}$)).}
  \end{center}
\end{frame}
  ...
  \begin{center}
    \begin{tabular}{|l|l|l|}
      ...
      $\ell$-\textsc{MultiCoverage} & $\min\{ j,\ell\}$ & $1-\frac{\ell^{\ell}e^{-\ell}}{\ell!}$ \\
       \textsc{Proportional Approval Voting} & $\sum_{i=1}^j \frac{1}{i}$ & $\alpha_{\varphi}(1) \simeq 0.7965\ldots$\\
      %\textsc{PAV} capped at $3$ & $\sum_{i=1}^{\min\{j,3\}} \frac{1}{i}$ & $\alpha_{\varphi}(1) \simeq 0.7910\ldots$ \\
      $p$-\textsc{Vehicle-Target Assignment} & $\frac{1-(1-p)^j}{p}$ & $\frac{1 - e^{-p}}{p}$  \\
...
\subsection{An efficient algorithm for $\mathrm{S}^{\mathrm{NS}}(W^{\otimes n},k_1,k_2)$ and inner bounds}
\begin{frame}{The multiple-access channel coding problem}
  \begin{itemize}
  \item \underline{Problem $\mathrm{S}(W,k_1,k_2)$:} send $k_1$ and $k_2$ messages through $W$ with best $\textcolor{blue}{e_1}:[k_1] \rightarrow \mathcal{X}_1$, $\textcolor{blue}{e_2}:[k_2] \rightarrow \mathcal{X}_2$, $\textcolor{blue}{d}:\mathcal{Y} \rightarrow [k_1] \times [k_2]$.
  ...
  \item \underline{Binary Adder Channel (BAC):} $W_{\text{BAC}} : (x_1,x_2) \mapsto x_1+x_2$
  \end{itemize}
\end{frame}
...
\begin{frame}{The broadcast channel coding problem}
    \begin{itemize}
  \item \underline{Problem $\mathrm{S}(W,k_1,k_2)$:} send $k_1$ and $k_2$ messages through $W$ with best $\textcolor{blue}{e}:[k_1] \times [k_2] \rightarrow \mathcal{X}$, $\textcolor{blue}{d_1}:\mathcal{Y}_1 \rightarrow [k_1], \textcolor{blue}{d_2}:\mathcal{Y}_2 \rightarrow [k_2]$.
...
\begin{frame}{Proof ideas: approximation algorithm and \textrm{NP}-hardness}
    ...
    \begin{column}{4cm}
      \begin{center}
        \begin{tikzpicture}[scale=0.9]
          ...
          \node (Y1) at (-1,-1.5) {$\mathcal{Y}_1$};
          \node (Y2) at (3,-1.5) {$\mathcal{Y}_2$};
          \node (X) at (1,0) {$\mathcal{X}$};
          \draw[decoration={brace,mirror,raise=15pt},decorate]  (0,0) -- (0,-3);
          \draw[decoration={brace,mirror,raise=15pt},decorate]  (2,-2.5) -- (2,-0.5);         
  ...
  \begin{itemize}
  \item \textrm{NP}-hard as equivalent to \textsc{SetSplitting} when $k_1=2,k_2=|\mathcal{Y}_2|$.
    \pause
    \bigskip
...
\begin{frame}{Proof ideas: non-signaling assisted capacity region}
  ...
  \item \underline{Approximation algorithm:}
    \begin{enumerate}
    \item Upper bound $f(G_W) \geq \mathrm{S}\only<3->{^{\textcolor{blue}{\mathrm{NS}}}}(W,k_1,k_2)$.
    \item Finds solution with value at least $\alert{C_{\text{apx}}} \cdot f(G_W)$
    \end{enumerate}
    \pause
  \item Implies $\mathrm{S}(W,k_1,k_2) \geq \alert{C_{\text{apx}}} \cdot f(G_W)\only<3->{ \geq \alert{C_{\text{apx}}} \cdot  \mathrm{S}^{\textcolor{blue}{\mathrm{NS}}}(W,k_1,k_2)}$.
    \pause
    \pause
    \begin{theorem}
      \label{theo:NSdet}
...
\begin{frame}{Social welfare reformulation and value query hardness}
  ...
  \item \underline{Reformulation:} take $k_2=|\mathcal{Y}_2|$, then BCC objective is:
    ...
    \begin{theorem}[Value query hardness]
