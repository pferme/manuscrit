...
\begin{frame}{Variables cachées et intrication quantique}
  \begin{center}
    Que se passe t-il si Alice et Bob partagent\ldots\only<2>{des \emph{variables cachées} ?}\\\bigskip
    \begin{tikzpicture}[auto, node distance=1.8cm,>=latex']
      \node [block] (A) {\includegraphics[scale=0.08]{Alice.png}};
      \node [above of=A] (x) {$x$};
      \draw [->] (x) -- (A);
      ...
      \only<2->{\node [circle, fill=white, draw=black] at (I) (Corr) {$\lambda$};
        \draw [snake=coil,segment aspect=0] (A) -- (Corr);
        \draw [snake=coil,segment aspect=0] (Corr) -- (B);}
    \end{tikzpicture}
  \end{center}
\end{frame}
