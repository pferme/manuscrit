...
\begin{frame}{Link to Generalized List-decoding}
  ...
  \item Success probability of size-$k$ code $S \subseteq \mathcal{X}$ is $\frac{1}{kt}\sum_{y \in \mathcal{Y}} \varphi(\abs{S}_y)$ for $\varphi$-list-decoding.
    \bigskip
    \pause
  \item Up to constant factor $\frac{1}{kt}$, same value as for $\varphi$-\textsc{MaxCoverage}: tight approximation follows for that class of channels.
  \item Open to extends approximation algorithm to all channels.
  \end{itemize}
\end{frame}
...
\begin{frame}{Our results}
  ...
  \begin{theo}[Main Result]
    ...
    For $\varphi(n) = o(n)$, it is \textrm{NP}-hard to approximate within a better ratio than $\alpha_\varphi + \varepsilon$, even restricted to unit weights and regular sets.
  \end{theo}
\end{frame}
